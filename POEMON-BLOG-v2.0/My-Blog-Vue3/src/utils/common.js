...
export default {
  ...
  faceReg(content) {
    content = content.replace(/\[[^\[^\]]+\]/g, (word) => {
      const index = constant.emojiList.indexOf(
    ...
    return new Promise((resolve) => {
      ...
      import("@/api").then(({ default: api }) => {
        api
          .getIpAddress()
          .then(async (res) => {
            if (!this.isEmpty(res.result[0].data[0].ip)) {
              ...
              try {
                ...
                if (
                  ...
                  // 动态导入 notify
                  import("@/utils/notify").then(({ notifyError }) => {
                    notifyError(
                      "由于设备隐私问题，无法获取您当前地点的天气信息"
                    );
                  });
                  return;
                }
                ...
                // API 请求失败时，静默处理，不显示错误提示
                console.warn("天气 API 请求失败:", error.message);
                resolve({ city: "", address: "", weather: "", tip: "" });
              }
            } else {
              // 动态导入 notify
              import("@/utils/notify").then(({ notifyError }) => {
                notifyError("由于设备隐私问题，无法获取您当前地点的天气信息");
              });
            }
          })
          .catch((error) => {
  ...
  countdown(time) {
    ...
    // 两个时间点的时间差(秒)
    const seconds = parseInt((time.getTime() - nowTime.getTime()) / 1000);
