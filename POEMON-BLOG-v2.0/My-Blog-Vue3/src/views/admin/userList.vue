...
const fuzzyUserType = ref(null);
const fuzzyUserStatus = ref(null);
...
const saveEdit = async () => {
  try {
    await api.changeUserType({
      userId: changeUser.value.id,
      userType: changeUser.value.userType,
    });
    handleClose();
    await getUsers();
    notifySuccess("修改成功！");
