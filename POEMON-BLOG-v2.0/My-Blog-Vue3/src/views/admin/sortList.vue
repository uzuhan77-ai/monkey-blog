...
const deleteHandle = (id, flagValue) => {
  if (flagValue === 1) {
    title.value = "分类";
  } else if (flagValue === 2) {
    title.value = "标签";
...
// 保存编辑
const saveEdit = async (flagValue) => {
  if (flagValue === "分类") {
    if (
    ...
    try {
      if ($common.isEmpty(sortForHttp.value.id)) {
        await api.saveSort(sortForHttp.value);
      } else {
        await api.updateSort(sortForHttp.value);
      }
      notifySuccess("保存成功！");
      await getSortInfo();
      handleClose();
    ...
    try {
      if ($common.isEmpty(labelForHttp.value.id)) {
        await api.saveLabel(labelForHttp.value);
      } else {
        await api.updateLabel(labelForHttp.value);
      }
      notifySuccess("保存成功！");
      await getSortInfo();
      handleClose();
...
// 表格中编辑分类和标签事件
const editSortAndLabel = (SortOrLabel, flagValue) => {
  if (flagValue === "编辑分类") {
    title.value = "编辑分类";
    flag.value = "分类";
    sortForHttp.value.id = SortOrLabel.id;
    sortForHttp.value.sortName = SortOrLabel.sortName;
    sortForHttp.value.sortDescription = SortOrLabel.sortDescription;
    sortForHttp.value.sortType = SortOrLabel.sortType;
    sortForHttp.value.priority = SortOrLabel.priority;
  } else {
    title.value = "编辑标签";
    flag.value = "编辑标签";
    labelForHttp.value.id = SortOrLabel.id;
    labelForHttp.value.sortId = SortOrLabel.sortId;
    labelForHttp.value.labelName = SortOrLabel.labelName;
    labelForHttp.value.labelDescription = SortOrLabel.labelDescription;
  }
  Dialog.value = true;
};
...
const getSortInfo = async () => {
  try {
    ...
    if (!$common.isEmpty(res.result[0])) {
      const filteredSortInfo = res.result[0].data.filter((item) => {
        return item.id !== 11;
      });
      sortInfo.value = filteredSortInfo;
    }
  } catch (error) {
    console.log(error);
  }
